...
const TimelineItem = memo(({ project, isLast, index }: { project: typeof projects[0]; isLast: boolean; index: number }) => (
  <motion.div
  ...
  >
    {!isLast && (
      <div className="absolute left-2.75 sm:left-3.75 top-3 -bottom-7.5 w-px bg-text-muted/20 group-hover:bg-text-muted/50 transition-colors" />
    )}
    <div className="absolute left-0 sm:left-1 top-3 w-5.5 h-5.5 rounded-full border border-custom bg-page flex items-center justify-center z-10">
...
export default function ProjectsPage() {
  ...
  const email = person.email; // ← from siteConfig
